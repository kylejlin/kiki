...
pub fn table_to_rust(table: &Table, file: ValidatedFile) -> Result<RustSrc, KikiErr> {
    ...
    let eof_src = create_unique_identifier("Eof", &mut used_identifiers);
    let quasitoken_src = create_unique_identifier("Quasitoken", &mut used_identifiers);
    let quasitoken_kind_src = create_unique_identifier("QuasitokenKind", &mut used_identifiers);
    ...
    let quasitoken_kind_from_token_match_arms_indent_3: String = file
        ...
        .map(|variant| {
            let name = &variant.dollarless_name;
            format!("{token_src}::{name}(_) => Self::{name},")
        })
...
where S: IntoIterator<Item = {token_src}> {{
    let mut tokens = src.into_iter()
        .map({quasitoken_src}::Token)
        .chain(std::iter::once({quasitoken_src}::Eof))
        .peekable();
    let mut states = vec![{state_src}::S0];
    let mut nodes: Vec<{node_src}> = vec![];
    loop {{
        let top_state = *states.last().unwrap();
        let top_token_kind = {quasitoken_kind_src}::from_quasitoken(tokens.peek().unwrap());
...
impl {quasitoken_kind_src} {{
    fn from_quasitoken(quasitoken: &{quasitoken_src}) -> Self {{
        match quasitoken {{
            Self::Token(token) => Self::from_token(token),
            Self::{eof_src} => Self::{eof_src},
        }}
    }}

    fn from_token(token: &{token_src}) -> Self {{
        match token {{
